...
pses_org <- read.csv(file.path(data_dir,"subset-7-sous-ensemble-7-1.csv"), na.strings = "9999") %>% 
  filter(LEVEL1ID %in% c(0,this_deptcode))

sector_list <- c(216, 340, 341, 342) 
...
for (this_unitcode in sector_list) {
  ...
  y_dim <- c(e = "Score 100",
             f = "Score 100")
  filename <- c(e = paste0("PSES2020@",this_dept_e,
                           " - Sector Line Chart - ",
                           this_unitcode," - ",
                           str_replace_all(this_sector[[lang]], "\\\\|/", ";"),
                           ".png"),
                f = paste0("SAFF2020@",this_dept_f,
  ...
  ggsave(file.path("plots",filename[[lang]]),
